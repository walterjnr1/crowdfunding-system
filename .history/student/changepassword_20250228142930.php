...
$q = "SELECT * FROM students where track_number='$track_number'";
  $q1 = $conn->query($q);
...
<?php $error='NEW Password and CONFIRM password not Matched';?>
  <?php } else {
   ...
   $sql = "update customer set `password`='$confirm_new' where track_number='$track_number'";
  $res = $conn->query($sql);
...
<title>Change Password| <?php echo $si;   ?></title>
