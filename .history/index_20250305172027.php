...
<body>
    ...
    <section class="campaigns">
        <div class="container">
            ...
            <div class="row">
            <?php
$sql = "  SELECT students.*,  requests.photo as Campaign_image, requests.*, ( SELECT SUM(donation_amount) FROM donations 
WHERE request_id = requests.id ) as total_donation FROM students INNER JOIN requests ON students.track_number = requests.student_id 
              ...
              <div class="col-md-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text">Raised: $<?php echo $total_donation;?> / Goal: N<?php echo number_format((float) $row['amount'] ,2); ?></p>
                            <button class="btn btn-success btn-lg" onClick="window.location.href='donate.php?requestid=<?php echo $row['requestid'] ?>&studentid=<?php echo $row['student_id'] ?>'">Donate</button>
                        </div>
                    </div>
                </div>
                <?php } ?>
