...
SELECT 
    ...
    requests.*, 
    COALESCE(SUM(payments.amount), 0) as total_donated
FROM 
    students 
INNER JOIN 
    requests ON students.track_number = requests.student_id
LEFT JOIN 
    payments ON requests.id = payments.request_id
WHERE 
    requests.id = '$request_id'  // Add this line to filter by request ID
GROUP BY 
    requests.id, 
    students.track_number
ORDER BY 
    requests.id ASC
";
              ...
              <div class="col-md-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-text">Request ID : <?php echo $row['id']; ?></p>
                            <p class="card-text">Raised: N<?php echo number_format((float) $row['total_donated'] ,2); ?> / Goal: N<?php echo number_format((float) $row['amount'] ,2); ?></p>
                            <button class="btn btn-success btn-lg" onClick="window.location.href='donate.php?<?php echo $row['requestid'] ?>&<?php echo $row['studentid'] ?>'">Donate</button>
                        </div>
                    </div>
                </div>
                <?php } ?>
